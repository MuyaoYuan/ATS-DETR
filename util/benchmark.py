...
@torch.no_grad()
def compute_gflops(model, dataset, approximated=True, cpu=False):
    if cpu:
        model.to('cpu')
    ...
    for img in tqdm.tqdm(images):
        ...
        with warnings.catch_warnings():
            ...
            res, _ , giga_flops_encoder= flop_count_without_warnings(model, (inputs,), )
        gflops = sum(res.values())
        gflops_list.append(gflops)
        giga_flops_encoder_list.append(giga_flops_encoder)
        imsize_list.append(list(img.shape))
        del res  # Manually release the `res` tensor
    ...
    print(f"Encoder GFLOPs : {np.array(giga_flops_encoder_list).mean()}")
    model.train()
    python_ops_mode_for_deform_attn(model, False)
    # return gflops
    # return gflops this is wrong in the original code
    return np.array(gflops_list).mean(), np.array(giga_flops_encoder_list).mean()
...
def flop_count_without_warnings(
    ...
    giga_flops_encoder = by_module["transformer.encoder.layers"] / 1e9
    giga_flops = defaultdict(float)
    for op, flop in flop_counter.by_operator().items():
        giga_flops[op] = flop / 1e9
    return giga_flops, flop_counter.unsupported_ops(), giga_flops_encoder
