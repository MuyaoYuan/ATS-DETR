...
def attn_map_to_flat_grid(spatial_shapes, level_start_index, sampling_locations, attention_weights):
    ...
    rev_spatial_shapes = torch.stack([spatial_shapes[..., 1], spatial_shapes[..., 0]], dim=-1) # hw -> wh (xy) [n_level, 2]
    ...
    zero = torch.zeros(*col_row_ll.shape[:-1], dtype=torch.int64, device=col_row_ll.device) # [N * n_layers * n_heads, Len_q * n_points, n_levels]
    one = torch.ones(*col_row_ll.shape[:-1], dtype=torch.int64, device=col_row_ll.device) # [N * n_layers * n_heads, Len_q * n_points, n_levels]
    col_row_lh = col_row_ll + torch.stack([zero, one], dim=-1) # [N * n_layers * n_heads, Len_q * n_points, n_levels, 2]
    col_row_hl = col_row_ll + torch.stack([one, zero], dim=-1)
    col_row_hh = col_row_ll + 1 # For Align

    margin_ll = (col_row_float - col_row_ll).prod(dim=-1) # [N * n_layers * n_heads, Len_q * n_points, n_levels] .prod -> .product(dim)
    ...
    flat_grid = torch.zeros(flat_grid_shape, dtype=torch.float32, device=attention_weights.device) #[N * n_layers * n_heads, L of feature]
    ...
    for col_row, margin in zipped:
        ...
        idx = col_row[..., 1] * spatial_shapes[..., 1] + col_row[..., 0] + level_start_index # [N * n_layers * n_heads, Len_q * n_points, n_levels]
    ...
    return flat_grid.reshape(N, n_layers, n_heads, -1) # [N, n_layers, n_heads, L of feature]
