...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(nn.Sequential(
                    ...
                    nn.GroupNorm(32, hidden_dim), # 将channel切分成许多组进行归一化
                ))
        ...
        if self.distill and self.adaption:
            self.adaption_layer = nn.Linear(hidden_dim, hidden_dim)
            nn.init.xavier_uniform_(self.adaption_layer.weight, gain=1)
            nn.init.constant_(self.adaption_layer.bias, 0)
    ...
    def forward(self, samples: NestedTensor):
        ...
        (hs, init_reference, inter_references, 
         ...
         backbone_mask_prediction, topk_idx_enc,
        ...
        out['topk_idx_enc'] = topk_idx_enc
            
        if self.use_enc_aux_loss:
            out['aux_outputs_enc'] = self._set_aux_loss(enc_inter_outputs_class, enc_inter_outputs_coord)
        
        out["sparse_token_nums"] = self.transformer.encoder.sparse_token_nums
        out["sparse_ratio"] = self.transformer.encoder.sparse_ratio_fact
        if self.adaptive:
            out["learnable_rate"] = self.transformer.encoder.sparse_ratio #[bs] / [bs, num_ratio]
        
        if self.distill:
            if self.adaption:
                out["memory_enc"] = self.adaption_layer(memory_enc)
            else:
                out["memory_enc"] = memory_enc
...
class SetCriterion(nn.Module):
    ...
    def loss_mask_prediction(self, outputs, targets, indices, num_boxes, layer=None):
        ...
        assert "sparse_token_nums" in outputs
        ...
        mask_prediction = outputs["backbone_mask_prediction"] #[bs, 1, L]
        loss_key = "loss_mask_prediction"

        sampling_locations_dec = outputs["sampling_locations_dec"] #[N, n_layers, Len_q, n_heads, n_levels, n_points, 2]
        ...
        flat_grid_attn_map_dec = attn_map_to_flat_grid(
            spatial_shapes, level_start_index, sampling_locations_dec, attn_weights_dec).sum(dim=(1,2)) #[bs, L]
        ...
        sparse_token_nums = outputs["sparse_token_nums"] # [bs, len of pruing_loc]
        num_topk = sparse_token_nums.max()

        topk_idx_tgt = torch.topk(flat_grid_attn_map_dec, num_topk)[1] #[bs, num_topk]
        
        target = torch.zeros_like(mask_prediction) #[bs, 1, L]
        
        for bid in range(target.shape[0]):
            for pid in range(target.shape[1]):
                target[bid,pid].scatter_(0, topk_idx_tgt[bid][:sparse_token_nums[bid,pid]], 1)
    ...
    def forward(self, outputs, targets):
        ...
        outputs_without_aux = {k: v for k, v in outputs.items() 
                               if k not in ['aux_outputs', 'enc_outputs', 'backbone_outputs', 'aux_outputs_enc']}
        ...
        if 'aux_outputs' in outputs: # aux output in decoder
        ...
        if 'enc_outputs' in outputs: # last layer of encoder output
        ...
        if 'backbone_outputs' in outputs: # backbone output
        ...
        if 'aux_outputs_enc' in outputs: # aux output in encoder
            for i, aux_outputs in enumerate(outputs['aux_outputs_enc']):
                indices = self.matcher(aux_outputs, targets) # aux_outputs['pred_logits'][bs, topk, n_classes]
                for loss in self.losses:
                    if loss in ['masks', "mask_prediction", "corr"]:
                        # Intermediate masks losses are too costly to compute, we ignore them.
                        continue
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        """
        out_logits, out_bbox = outputs['pred_logits'], outputs['pred_boxes'] # [bs, n_query, n_classes], [bs, n_query, 4]

        assert len(out_logits) == len(target_sizes) # bs
        assert target_sizes.shape[1] == 2

        prob = out_logits.sigmoid() # [bs, n_query, n_classes]
        topk_values, topk_indexes = torch.topk(prob.view(out_logits.shape[0], -1), 100, dim=1) # [bs, n_query*n_classes] -> [bs, 100]
        scores = topk_values # [bs, 100]
        topk_boxes = topk_indexes // out_logits.shape[2] # devide n_classes to get the query_idx
        labels = topk_indexes % out_logits.shape[2] # mod n_classes to get the label_idx
        boxes = box_ops.box_cxcywh_to_xyxy(out_bbox) # c_x, c_y, w, h -> x_min, y_min, x_max, y_max
        boxes = torch.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4)) # [bs, 100, 4]

        # and from relative [0, 1] to absolute [0, height] coordinates
        img_h, img_w = target_sizes.unbind(1)
        scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1) # [bs, 4]
        boxes = boxes * scale_fct[:, None, :] # [bs, 100, 4] x_min, y_min, x_max, y_max to absolute [0, height] coordinates
